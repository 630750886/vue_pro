<template>
  <div>
    <el-dialog :title="title" :visible.sync="dialogVisible" v-if="dialogVisible" width="30%" @close="dialogClose()" :close-on-click-modal="false">
      <el-form :model="form" ref="formRef" :rules="formRules" label-width="85px">
        <el-form-item label="制度类型" prop="fileTypeKeyId">
           <el-select v-model="form.fileTypeKeyId" placeholder="请选择" @change="getsealType">
					<el-option
					v-for="item in RegulationType"
					:key="item.id"
					:label="item.name"
					:value="item.id">
					</el-option>
        ...
        <el-form-item label="制度编号" prop="documentCode">
          <el-input v-model="form.documentCode" clearable></el-input>
        </el-form-item>
        <el-form-item label="制度名称" prop="documentName">
        ...
        <el-form-item label="制度执行时间" prop="userName">
          <el-date-picker
					v-model="form.documentBeginDate"
					type="date"
					placeholder="选择日期"
					format="yyyy/MM/dd"
					value-format="yyyy/MM/dd">
...
<!--        <el-form-item label="制度涉及相关组织" prop="gzaboutorgDesc">-->
<!--         <el-input-->
<!--            type="textarea"-->
<!--            :rows="2"-->
<!--            placeholder="请输入"-->
<!--            v-model="form.gzaboutorgDesc">-->
<!--          </el-input>-->
<!--        </el-form-item>-->
        <el-form-item label="制度附件" prop="documentAttachFile">
...
<script>
  ...
  export default {
    ...
    methods: {
      getsealType(data){
        console.log(data);
        let ditem= this.type.find((item,index)=>{
          if(item.id=== data){
          return item;
        }
          })
