<template>
	<el-dialog :visible.sync="dialogVisible"  v-if="dialogVisible" width="40%" :title="title1" :close-on-click-modal="false">
		<el-form :model="form" ref="formRef" :rules="formRules" label-width="20%">
			<el-form-item label="电子印章名称" prop="electronicSealName">
				<el-input v-model="form.electronicSealName" placeholder="电子印章名称" clearable></el-input>
			</el-form-item>
		</el-form>
	...
	export default {
		...
		data: function() {
			...
			return {
				...
				//表单验证规则
				formRules: {
					electronicSealName:[{ required: true, message: '请输入必填项', trigger: 'blur' }]
				}
			}
		},
