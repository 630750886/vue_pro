...
<!--        <el-table-column prop="publicitydateTime" label="创建日期" align="center"><template slot-scope="scope">{{scope.row.createddateTime|datefmt('YYYY/MM/DD')}}</template></el-table-column>-->
...
<!--              {label:'更正截图',prop:'publicityChangeImageFile',type:'file',upFile:'true'},-->
<!--            ]"-->
...
<!--            <el-button type="text" @click="$refs.perfectRef.openDialog(scope.row)">完善发布信息</el-button>-->
            ...
            <el-button type="primary" plain size="mini" @click="delDemand(scope.row)">删除</el-button>
          </template>
        </el-table-column>
    ...
    <div class="dialog">
<!--      title="新增编制及发布"-->
      <add-deman-dialog  width="30%" ref="addRef" @getFormData="networkAddData"></add-deman-dialog>
...
<!--      <perfect-info-dialog title="完善发布信息" width="50%" ref="perfectRef"></perfect-info-dialog>-->
      <objection-handling-dialog title="异议处理登记" width="30%" ref="handingRef"
...
<script>
  ...
  import getWeb from "../dialog/DemandNoticeTable/getWeb.vue";
  import {
    getPretrial,
    addPretrial,
    upPretrial,
