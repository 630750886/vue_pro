  ...
  <el-dialog :title="title" :visible.sync="dialogVisible"  v-if="dialogVisible" :width="width" @close="dialogClose" :close-on-click-modal="false">
    <el-form :model="form" ref="formRef" :rules="formRules" label-width="140px">
      ...
      <el-table :data="workPlanList" stripe :show-header="false" >
        <el-table-column width="100px" align="right">服务名称：</el-table-column>
       ...
       <!-- <el-table-column>
          <template slot-scope="scope">
            <template v-if="scope.row.isShowPlanServiceDays">
              <el-table-column min-width="75px" align="right">计划时间：</el-table-column>
              <el-table-column prop="planServiceDays" min-width="100px" align="left"></el-table-column>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="prop" min-width="75px" align="right">
          <template slot-scope="scope">
            <span v-if="scope.row.workItemContext!==''">工作类容：</span>
          </template>
        </el-table-column>
    ...
    <div>
      ...
      :cancelBtn="{show:false}" @okBtn="submitFormData"></dialog-button>
    </div>
  </el-dialog>
</template>
<script>
