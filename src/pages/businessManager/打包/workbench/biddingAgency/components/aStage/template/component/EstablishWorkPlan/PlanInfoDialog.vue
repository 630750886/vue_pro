<template>
  ...
  <el-dialog :title="title" :visible.sync="dialogVisible" v-if="dialogVisible" :width="width" @close="dialogClose" :close-on-click-modal="false">
    <el-form :model="form" ref="formRef" :rules="formRules" label-width="140px">
      ...
      <el-table :data="workPlanList" stripe :show-header="false" border>
        ...
        <!--<el-table-column min-width="75px" align="right">
          <template slot-scope="scope">
            <template v-if="scope.row.planServiceDays!==''">
              计划时间：
            </template>
          </template>

        </el-table-column>
        <el-table-column prop="planServiceDays" min-width="100px" align="left">
          <template slot-scope="scope">
            <template v-if="scope.row.planServiceDays!==''">
              {{scope.row.planServiceDays}}
            </template>
          </template>
        </el-table-column>
